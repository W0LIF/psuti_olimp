...
export function TransactionFilters({ 
  ...
  const applyFilters = () => {
    ...
    if (localFilters.dateRange === 'custom') {
      filtersToApply.startDate = tempStartDate || null;
      filtersToApply.endDate = tempEndDate || null;
    }
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <div className="flex gap-2">
          <button
            onClick={() => updateLocalFilter('type', 'all')}
          ...
          <button
            onClick={() => updateLocalFilter('type', 'income')}
            className={`flex-1 py-2 rounded-lg transition-all ${
          ...
          <button
            onClick={() => updateLocalFilter('type', 'expense')}
            className={`flex-1 py-2 rounded-lg transition-all ${
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {DATE_RANGE_OPTIONS.map((option) => (
            <button
              key={option.value}
              onClick={() => updateLocalFilter('dateRange', option.value)}
        ...
        <div className="space-y-3 animate-slide-down">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-input-background rounded-lg border border-transparent focus:border-primary focus:outline-none transition-colors"
            />
          </div>
          <div>
      ...
      {categories.length > 0 && (
        <div>
          ...
          <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto pb-1">
            <button
              onClick={() => updateLocalFilter('category', null)}
            ...
            {categories.map((cat) => (
              <button
                key={cat}
                onClick={() => updateLocalFilter('category', cat)}
                className={`px-3 py-1 rounded-full text-xs transition-all ${
      ...
      <div className="flex gap-3">
        {hasActiveFilters && (
          <button
            ...
            className="flex-1 px-4 py-2 bg-muted text-muted-foreground rounded-lg hover:bg-muted/80 transition-colors text-sm"
          >
            Сбросить
          </button>
        )}
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-border p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-base font-medium">Фильтрация</h3>
          {activeFiltersCount > 0 && (
