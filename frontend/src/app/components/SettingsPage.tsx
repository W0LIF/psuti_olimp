...
interface SettingsPageProps {
  userName: string;
  onUpdateProfile: (name: string, email: string, password: string) => void;
}
...
export function SettingsPage({ userName, onUpdateProfile }: SettingsPageProps) {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    onUpdateProfile(name, email, newPassword || currentPassword);
    setSaved(true);
    setNewPassword('');
    setConfirmPassword('');
    setCurrentPassword('');
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        <h2 className="mb-2">Настройки профиля</h2>
        <p className="text-muted-foreground">
          Управляйте своим профилем и безопасностью
        </p>
      </div>

      {saved && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-start gap-3 animate-slide-down">
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-border p-6">
          ...
          <div className="space-y-4">
            <div>
              <label className="block mb-2">Текущий пароль</label>
              <input
                type="password"
