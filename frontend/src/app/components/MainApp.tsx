                ...
                <div key={category} className="bg-muted dark:bg-gray-700 rounded-lg p-3">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Потрачено: {format(spent)}</p>
                    </div>
                    {editingCategory === category ? (
                      ...
                      <button onClick={() => { setEditingCategory(category); setTempLimit(limit.toString()); }} className="text-sm text-primary hover:underline">{limit > 0 ? `${format(limit)}` : 'Не задан'} ✏️</button>
                    )}
                  </div>
                </div>
              ))}
...
function CategoryBudgetProgress({ category, limit, spent }: CategoryBudget) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl p-3 shadow-sm border border-border">
      ...
      {isOverLimit && <p className="text-xs text-red-600 dark:text-red-400 mt-1 flex items-center gap-1">Превышение на {format(Math.abs(remaining))}</p>}
      {isNearLimit && <p className="text-xs text-yellow-600 dark:text-yellow-400 mt-1">Осталось: {format(remaining)}</p>}
    </div>
  );
}

export function Dashboard({ userName, currentMonth, transactions, onAddTransaction, onDeleteTransaction }: DashboardProps) {
  const { format } = useCurrency();
    ...
    <div className="space-y-6">
        ...
        <div className="bg-red-50 dark:bg-red-950/40 border-l-4 border-red-500 p-4 rounded-xl">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm text-red-700 dark:text-red-300">Вы превысили лимит на {format(Math.abs(overallLimit - expenses))}</p>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="bg-yellow-50 dark:bg-yellow-950/40 border-l-4 border-yellow-500 p-4 rounded-xl">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm text-yellow-700 dark:text-yellow-300">Потрачено {Math.round(overallPercentage)}% от лимита. Осталось: {format(overallLimit - expenses)}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="hidden lg:block"><TransactionForm onAdd={onAddTransaction} categories={customCategories} onAddCategory={handleAddCategory} onEditCategory={handleEditCategory} onDeleteCategory={handleDeleteCategory} /></div>
      ...
      {showTransactionForm && (
        <div className="lg:hidden fixed inset-0 bg-black/50 z-50 flex items-end" onClick={() => setShowTransactionForm(false)}>
          <div className="bg-white dark:bg-gray-800 rounded-t-3xl w-full max-h-[90vh] overflow-y-auto animate-slide-up" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-4"><TransactionForm onAdd={onAddTransaction} categories={customCategories} onAddCategory={handleAddCategory} onEditCategory={handleEditCategory} onDeleteCategory={handleDeleteCategory} /></div>
          </div>
        </div>
      )}
    </div>
